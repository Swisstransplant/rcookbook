...
We can use the function `num2date()` to convert numbers into dates. There is also a built-in filter that recognizes the alternative format, which can also be modified via the option, she `?num2date`.
...
```{r}
data = as.data.frame(read_xlsx(path = "../data/dates.xlsx", col_types = "text"))
data$mydate[1] = date2num("2024-04-30")
data$mydate = num2date(data$mydate)
data
```
